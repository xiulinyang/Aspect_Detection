...
def get_aspect(pred_dir, num):
  aspect_list =[]
  with open(pred_dir, 'r') as f:
    lines = f.readlines()
    for line in lines:
      if '<startoftext>' in line:
        line = re.sub(r'predicted aspects: <startoftext>', '', line)
        line = re.sub('�', '', line)
        line = line.split('Aspect: ')[num+1].split('.')[0]
...
def main():
    ...
    name = f'{model_name}_with_{num}_prompt_3532.txt'
    ...
    predicted_aspect = get_aspect(name, num)
